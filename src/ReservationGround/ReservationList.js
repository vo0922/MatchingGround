...
function ReservationCard({history}) {
  ...
  const handleSelectcity = (e) => {
    ...
    .then((res)=>{
      setcountrymenu(
        res.map((data) =>
        <MenuItem key={data.properties.sig_kor_nm} value={data.properties.sig_kor_nm}>{data.properties.sig_kor_nm}</MenuItem>)
      )
    });
    }
  }
  ...
  return (
    ...
    <Container maxWidth="md" style={{marginTop:10,}}>
      ...
      <Paper component="form" style={{backgroundColor:"#F3F3F3",marginLeft:10,borderRadius:30}}>
      <InputBase
      ...
      />
      <IconButton type="button" onClick={submitSearch} aria-label="search" style={{padding:10}}>
        <SearchIcon />
      </IconButton>
      </Paper>
        </Grid>
